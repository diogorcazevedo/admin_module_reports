...
export default function SlideGems({product, gems}) {
        ...
        <>
            ...
            <button onClick={() => setOpen(true)}
                ...
                <PencilSquareIcon
                    ...
                    aria-hidden="true"
                />
                <span className="text-gray-500 group-hover:text-gray-700 text-xs">Gemas</span>
            </button>
                                                        ...
                                                        <form onSubmit={submit}>
                                                            <div className="max-w-2xl mx-auto px-4 lg:max-w-none lg:px-0">
                                                                <div className="mt-6 grid grid-cols-2 gap-4">
                                                                    <div className="">
                                                                        <label htmlFor="gem_id" className="block text-sm font-medium text-gray-700">
                                                                            Gemas
                                                                        </label>
                                                                        <select name="gem_id"
                                                                                required="required"
                                                                                id="gem_id"
                                                                                onChange={e => setData('gem_id', e.target.value)}
                                                                                value={data.gem_id}
                                                                                className=" block focus:ring-teal-500 focus:border-teal-500 w-full shadow-sm  sm:text-sm border-gray-300 rounded-md">
                                                                            <option>-</option>
                                                                            {gems.map((gem, index) => {
                                                                                return (
                                                                                    <option key={index} value={gem.id}>
                                                                                        {gem.name}
                                                                                    </option>
                                                                                );
                                                                            })}
                                                                        </select>
                                                                        {errors.gem_id && <div className="text-red-600">{errors.gem_id}</div>}
                                                                    </div>
                                                                </div>
                                            ...
                                            <div className=" px-6">
                                                <table className="border min-w-full divide-y divide-x divide-gray-200">
                                                    ...
                                                    <tr className="divide-x divide-y divide-gray-200">
                                                        <th  width="40%" className="text-gray-900 p-2">Produtos</th>
                                                        <th  width="40%" className="text-gray-900 p-2">Quantidade (unidades)</th>
                                                    ...
                                                    {product.gems?.map((data) => (
                                                        <tr key={data.id} className="divide-x divide-y divide-gray-200">
                                                            ...
                                                            <td className="text-sm p-2 text-center">
                                                                <Link href={route('product.gem.remove',{product_gem: data.id})}
