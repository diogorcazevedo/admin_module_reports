...
function menuItems(year,get_route){
    let menuItems = [];
    let y = year;
    ...
    for (let i = 1; i < 13; i++) {
        menuItems.push(
            <Menu.Item key={i}>
                {({ active }) => (
                    <Link
                        href={route(get_route,{year:y,month:i})}
...
export default function SlideGetRouteAndYearAndMonth({get_route,label=""}) {
