...
import SlideCategoryProductsSortByTotal from "@/Pages/Report/Category/Components/SlideCategoryProductsSortByTotal";
import SlideCategoryUsersSortByTotal from "@/Pages/Report/Category/Components/SlideCategoryUsersSortByTotal";

export default function Sales({data,month,sales_total}) {

    const {auth} = usePage().props
    const { errors } = usePage().props
    const sorted = orderBy(data, ["category_total"], 'desc');
    let count = 0
                                ...
                                <table className="border mt-2 min-w-full divide-y divide-x divide-gray-200">
                                    ...
                                    {Object.values(sorted).map((item,index) => (
                                        <tr key={index} className="divide-x divide-y divide-gray-200">
                                            ...
                                            <td>
                                                <SlideCategoryProductsSortByTotal category={item?.category} products={item.pd} />
                                            </td>
                                            <td>
                                                <SlideCategoryUsersSortByTotal category={item?.category} users={item.users} />
                                            </td>
                                        </tr>
                                    ))}
                                    </tbody>
