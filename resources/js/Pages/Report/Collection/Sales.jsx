...
import SlideCollectionProductsSortByTotal from "@/Pages/Report/Collection/Components/SlideCollectionProductsSortByTotal";
import SlideCollectionUsersSortByTotal from "@/Pages/Report/Collection/Components/SlideCollectionUsersSortByTotal";
export default function Sales({data,month,sales_total}) {

    const {auth} = usePage().props
    const { errors } = usePage().props
    const sorted = orderBy(data, ["collection_total"], 'desc');
    let count = 0
                                ...
                                <table className="border mt-2 min-w-full divide-y divide-x divide-gray-200">
                                    ...
                                    {Object.values(sorted).map((item,index) => (
                                        <tr key={index} className="divide-x divide-y divide-gray-200">
                                            ...
                                            <td className="text-xs p-2">{item?.collection}</td>
                                            <td className="text-xs p-2"> {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(item?.collection_total) }</td>
                                            <td className="text-xs p-2">{item?.orders_count}</td>
                                            <td>
                                                <SlideCollectionProductsSortByTotal collection={item?.collection} products={item.pd} />
                                            </td>
                                            <td>
                                                <SlideCollectionUsersSortByTotal collection={item?.collection} users={item.users} />
                                            </td>
                                        </tr>
                                    ))}
                                    </tbody>
