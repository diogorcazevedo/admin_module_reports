...
import SlideStateUsersSortByOrders from "@/Pages/Report/City/Components/SlideStateUsersSortByOrders";

export default function Sales({data,month}) {

    const {auth} = usePage().props
    const { errors } = usePage().props
    const sorted = orderBy(data, ["user_count"], 'desc');
                            ...
                            <div className="px-4 py-6 sm:px-6 lg:pl-8 xl:flex-1 xl:pl-6">
                                <table className="border mt-2 min-w-full divide-y divide-x divide-gray-200">
                                    ...
                                    <tr className="divide-x divide-y divide-gray-200">
                                        <th  width="5%" className="text-gray-900 p-2"> UF</th>
                                        <th  width="30%" className="text-gray-900 p-2"> Cidade</th>
                                    ...
                                    {Object.values(sorted).map((item,index) => (
                                        <tr key={index} className="divide-x divide-y divide-gray-200">
                                            ...
                                            <td>
                                                <SlideStateUsersSortByOrders state={item?.state} users={item.users} />
                                            </td>
                                        </tr>
                                    ))}
                                    </tbody>
