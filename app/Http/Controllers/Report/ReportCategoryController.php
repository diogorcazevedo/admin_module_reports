...
class ReportCategoryController extends Controller
{
    ...
    public function index(){
        ...
        return Inertia::render('Report/Category/Index',[
         //   'orders'=>$orders,
        //    'total'=>$total,
        ]);
    }
    public function year_sales($year = NULL ){
        ...
        $categories = $products->unique('category_id');
        $data = array();
        $sales_total = 0;

        foreach ($categories as $i => $category){

            $products   = Product::ofOrdersCategoryYear($category->category_id,$year)->get();
            $users      = User::ofOrdersCategoryYear($category->category_id,$year)->get();
            $cl         = Category::find($category->category_id);
            ...
            foreach ($products as $k => $product) {
                ...
                $category_total   = $category_total + $total;
            ...
            $data[$i]['category_total']     =   $category_total;
            $data[$i]['orders_count']       =   $orders_count;
            $data[$i]['users_count']        =   $users_count;

        }

        $data = collect($data)->sortBy('category_total')->reverse()->toArray();


        return Inertia::render('Report/Category/Sales',[
            'data'              =>$data,
            'month'             =>date('m'),
    ...
    public function year_quantity($year = NULL ){
        ...
        return Inertia::render('Report/Category/Quantity',[
            'data'              =>$data,
            'month'             =>date('m'),
            'sales_total'       =>$sales_total,
    ...
    public function month_sales($year = NULL, $month = NULL ){
        ...
        foreach ($categories as $i => $category){
            ...
            $products   = Product::ofOrdersCategoryYearAndMonth($category->category_id,$year,$month)->get();
            $users      = User::ofOrdersCategoryYearAndMonth($category->category_id,$year,$month)->get();
            $cl         = Category::find($category->category_id);
