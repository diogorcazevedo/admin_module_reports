...
use App\Http\Services\UserImageService;
...
class UserController extends Controller
{
    ...
    public function images(User $user)
    {
        ...
        $types = ImageType::where('sessao_id',2)->get();
        $images = $user->images();
        $images = $images->with('imageType')->get();

        return Inertia::render('Users/Images',[
            'images'=>$images,
            'user'=>$user,
            'types'=>$types,
        ]);
    ...
    {
        $this->userImageService->imageStore($request,$user);
        return redirect()->back()->with('message', 'Operação realizada com sucesso');
    }

    public function imageDestroy($image)
    {
        $this->userImageService->imageDestroy($image);
        return redirect()->back()->with('message', 'Operação realizada com sucesso');
    }
